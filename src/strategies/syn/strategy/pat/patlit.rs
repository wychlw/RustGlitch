...
pub struct PatLitStrategy;
impl Default for PatLitStrategy {
    fn default() -> Self {
        Self
    }
}
impl FuzzerStrategyImpl<Pat> for PatLitStrategy {
    fn do_gen(&mut self) -> Result<Pat, Box<dyn Error>> {
        let res = do_gen_name!(Expr, ExprLitStrategy)?.do_gen()?;
        let res = match res {
            Expr::Lit(r) => r,
            _ => unreachable!(),
        };
...
register_strategy!(PatLitStrategy, Pat, Pat::Lit(..));
